...
fn main() {
    ...
    println!("Before create handle A {}", Utc::now().timestamp());
    let handle_a = thread::spawn(move || {
        println!("Thread A created {}", Utc::now().timestamp());
        pause(2);
    ...
    // pause_ms(1); // Make sure Thread A has time to get going before we spawn Thread B
    pause(1);

    println!("Before create handle B {}", Utc::now().timestamp());
    let handle_b = thread::spawn(move || {
        println!("Thread B created {}", Utc::now().timestamp());
        pause(0);
    ...
    println!("Before print rx {}", Utc::now().timestamp());
    for msg in rx {
        println!("Main thread: Received {}", msg);
    }
    println!("After print rx {}", Utc::now().timestamp());
